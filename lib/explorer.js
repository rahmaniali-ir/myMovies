...
function checkMoviesCount(callback) {
    if(allMoviesLength === 0) return;
    console.log(allMoviesLength, allMoviesCount, pendingMovies, newMoviesCount);
...
function checkForFiles(folder, callback) {
    ...
    if(allMoviesLength === undefined) {
        allMoviesLength = Number(files.length);
    } else {
        allMoviesLength += Number(files.length);
    }
    ...
    targets.forEach(target => {
        if(fs.existsSync(target.path)) {
            checkForFiles(target.path, addedMovies => {
                console.log(addedMovies, 'added');
                targetsCount++;
                newMovies += addedMovies;
                
                console.log('targets', targetsCount, targets.length);
